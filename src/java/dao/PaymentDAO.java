...
public class PaymentDAO {
    public void addPayment(PaymentModal payment) {
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement ps = conn.prepareStatement(
                "INSERT INTO payments (amount, payment_date, student_id, course_id, section_id, payment_type, description, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', GETDATE())")) {
    ...
    public static List<PaymentModal> getPendingPayments() {
        ...
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement ps = conn.prepareStatement("SELECT * FROM payments WHERE status = 'pending'")) {
    ...
    public static void updateStatus(int paymentId, String status) {
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement ps = conn.prepareStatement("UPDATE payments SET status = ? WHERE id = ?")) {
